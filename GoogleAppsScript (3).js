...
function updateWalletTracking(walletAddr, ordererName, qty, amount, timestamp) {
  ...
  if (foundRow > 0) {
    ...
    var stakingAmount = parseFloat(summarySheet.getRange(foundRow, 7).getValue()) || 0;
    
    var newQty = prevQty + qty;
    var orderLimit = Math.floor(stakingAmount / 100);
    var remaining = orderLimit - newQty;
    var status;
    if (stakingAmount === 0) { status = '미설정'; }
    else if (newQty > orderLimit) { status = '초과'; }
    else { status = '정상'; }
    ...
    // 7열(스테이킹금액)은 관리자 수기 입력이므로 건드리지 않음
    summarySheet.getRange(foundRow, 8).setValue(orderLimit);
    summarySheet.getRange(foundRow, 9).setValue(remaining);
    summarySheet.getRange(foundRow, 10).setValue(status);
    summarySheet.getRange(foundRow, 11).setValue(timestamp);
    
    applyWalletRowColor(summarySheet, foundRow, status);
  } else {
    // 신규 지갑 등록
    var newRow = [
      walletAddr,
    ...
    var richText = SpreadsheetApp.newRichTextValue()
      ...
      .build();
    summarySheet.getRange(lastRow, 1).setRichTextValue(richText);
    summarySheet.getRange(lastRow, 1, 1, 11).setBackground('#fff8e1');
  }
...
function buildWalletTracking() {
  ...
  if (oldSheet && oldSheet.getLastRow() > 1) {
    ...
    for (var o = 1; o < oldData.length; o++) {
      ...
      var staking = parseFloat(oldData[o][6]) || 0; // G열: 스테이킹금액
      if (addr && staking > 0) {
  ...
  for (var s = 0; s < sheetNames.length; s++) {
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      if (!w.orders[parentOrder]) {
        w.orders[parentOrder] = true;
  ...
  for (var k = 0; k < walletKeys.length; k++) {
    ...
    var orderLimit = Math.floor(staking / 100);
    var remaining = orderLimit - w.totalQty;
    var status;
    if (staking === 0) { status = '미설정'; }
    else if (w.totalQty > orderLimit) { status = '초과'; }
    else { status = '정상'; }
  ...
  if (summaryRows.length > 0) {
    summarySheet.getRange(2, 1, summaryRows.length, 11).setValues(summaryRows);
    
    // 지갑주소 링크 + 행 색상 적용
    for (var r = 0; r < summaryRows.length; r++) {
      var addr = summaryRows[r][0];
      if (addr && addr.indexOf('0x') === 0) {
        var rt = SpreadsheetApp.newRichTextValue()
          .setText(addr)
          .setLinkUrl('https://bscscan.com/address/' + addr)
          .build();
        summarySheet.getRange(r + 2, 1).setRichTextValue(rt);
      }
      applyWalletRowColor(summarySheet, r + 2, summaryRows[r][8]);
    }
  }
...
// 행 색상 적용 헬퍼
function applyWalletRowColor(sheet, row, status) {
  var numCols = 11;
  if (status === '초과') {
    sheet.getRange(row, 1, 1, numCols).setBackground('#ffcdd2').setFontColor('#b71c1c');
  } else if (status === '미설정') {
    sheet.getRange(row, 1, 1, numCols).setBackground('#fff8e1').setFontColor('#e65100');
  } else {
    sheet.getRange(row, 1, 1, numCols).setBackground(null).setFontColor(null);
  }
}
...
function recalcWalletStatus() {
  ...
  for (var i = 0; i < data.length; i++) {
    var row = data[i];
    var totalQty = parseInt(row[4]) || 0;    // E열: 총주문수량
    var staking = parseFloat(row[6]) || 0;   // G열: 스테이킹금액
    var orderLimit = Math.floor(staking / 100);
    var remaining = orderLimit - totalQty;
    var status;
    
    if (staking === 0) { status = '미설정'; }
    else if (totalQty > orderLimit) { status = '초과'; }
    else { status = '정상'; }
    
    var r = i + 2;
    sheet.getRange(r, 8).setValue(orderLimit);   // H열: 주문가능수량
    sheet.getRange(r, 9).setValue(remaining);     // I열: 잔여수량
    sheet.getRange(r, 10).setValue(status);       // J열: 상태
    
    applyWalletRowColor(sheet, r, status);
    updated++;
  }
  
  Logger.log('=== ' + updated + '건 상태 재계산 완료 ===');
}
